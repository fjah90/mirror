    ...
    public function listado(Request $request)
    {
      ...
      if ($request->id == 'Todos') {
        ...
        if ($request->anio == 'Todos') {
            $proyectos = ProyectoAprobado::with('cotizacion','cliente','cotizacion.cuenta_cobrar','proyectos_aprobados.cotizacion.user')->get();
        }
        else{
            $anio = Carbon::parse($request->anio);
            $proyectos = ProyectoAprobado::whereBetween('created_at', [$inicio, $anio])->with('cotizacion','cliente','cotizacion.cuenta_cobrar','proyectos_aprobados.cotizacion.user')->get();
        }
        ...
        if ($request->anio == 'Todos') {
            $user = User::with('proyectos_aprobados.cotizacion', 'proyectos_aprobados.cliente','proyectos_aprobados.cotizacion.cuenta_cobrar','proyectos_aprobados.cotizacion.user')->find($request->id);
